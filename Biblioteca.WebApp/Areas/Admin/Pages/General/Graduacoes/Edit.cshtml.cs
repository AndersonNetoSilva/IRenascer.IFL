...
{
    public class EditModel : CrudPageModel<Graduacao, IGraduacaoRepository>
    {
        ...
        public EditModel(IGraduacaoRepository repository,
        ...
        public async Task<IActionResult> OnGetAsync(int? id)
        {
            ...
            var graduacao = await _repository.GetForUpdateAsync(id);
            ...
            GraduacaoVM = new GraduacaoVM
            {
                ...
                Encerrada = graduacao.Encerrada,
                HorarioId = graduacao.HorarioId,
                ModalidadeId = graduacao.ModalidadeId
            };

            GraduacaoAtletaVM = graduacao.GraduacaoAtletas
                                    .Select(p => new GraduacaoAtletaVM
                                    {
                                        Id = p.Id,
                                        FaixaNova   = p.FaixaNova,
                                        AtletaId    = p.AtletaId,
                                        GraduacaoId = graduacao.Id,
                                        NotaEscrita = p.NotaEscrita,
                                        NotaPratica = p.NotaPratica,
                                        Aprovado    = p.Aprovado.Value,
                                    })
                                    .ToList();

            foreach (var item in GraduacaoAtletaVM)
            {
                if (item.AtletaId >0)
                {
                    var atleta =  _atletaRepository.Query().Where( x=> x.Id == item.AtletaId).FirstOrDefault();

                    if (atleta != null)
                    {
                        item.Atleta = atleta.Nome;
                        item.FaixaAtual = atleta.GraduacaoAsString;

                        if (item.NotaEscrita.HasValue)
                            item.NotaEscritaAsString = item.NotaEscrita.Value.ToString(new System.Globalization.CultureInfo("pt-BR"));

                        if (item.NotaPratica.HasValue)
                            item.NotaPraticaAsString = item.NotaPratica.Value.ToString(new System.Globalization.CultureInfo("pt-BR"));
                    }
                }                
            }

            GraduacaoVM.GraduacaoAtletasVM = GraduacaoAtletaVM;
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                IFL.WebApp.Areas.Admin.Pages.General.Graduacoes.CreateModel.ValidarValor(GraduacaoVM);
                Graduacao graduacao = SetGraduacao(GraduacaoVM);

                await _graduacaoService.UpdateAsync(graduacao, GraduacaoVM.GraduacaoAtletasVM);
            }
